...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/temp-free-wan-default.sh apply   # prefer enp63s0 (removes ens4 defaults)
  bash scripts/temp-free-wan-default.sh restore # restore from latest backup
  bash scripts/temp-free-wan-default.sh status  # show default routes

Requires sudo (broker-sudo.py or ssh -t). Reversible via restore.
EOF
}
