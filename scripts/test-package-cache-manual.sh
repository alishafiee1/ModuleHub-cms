#!/usr/bin/env bash
# purpose --- Manual smoke test for package cache using phase4 fixture ZIP ------
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_DIR="${MODULEHUB_APP_DIR:-$(cd "${SCRIPT_DIR}/.." && pwd)}"
ZIP_PATH="${APP_DIR}/tests/fixtures/modules/phase4-cache-test.zip"
CMS_URL="${MODULEHUB_CMS_URL:-http://127.0.0.1:4000}"

if [[ ! -f "${ZIP_PATH}" ]]; then
  echo "[test-package-cache] Building fixture ZIP..."
  bash "${SCRIPT_DIR}/build-phase4-test-zip.sh"
fi
...
if [[ -n "${MODULE_ID_1}" ]]; then
  ...
  bash "${SCRIPT_DIR}/verify-phase4-cache.sh" "${MODULE_ID_1}" "${MODULE_ID_2:-}"
fi
