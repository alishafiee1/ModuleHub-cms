...
| ZIP flat چطور؟ | `build-package-cache-fixture-zip.ps1` یا داخل پوشه انتخاب همه → ZIP |

### تست

fixture: [`tests/fixtures/modules/package-cache-test/`](../tests/fixtures/modules/package-cache-test/) — `package.json` بدون `node_modules`.
...
| PATCH | `/admin/folder/:folderId/cards` | به‌روزرسانی `cardGrid` / `cardGridTablet` / `cardGridMobile` / `cardBackground` / `canvasGridRows*` |
...
| فیلد | توضیح |
|------|--------|
| `cardGrid` | `{ col, row, colSpan, rowSpan }` — چیدمان PC (desktop) |
...
**بک‌اند:** `core/src/modules/home-layout/` — `folder-cards-update.ts` · `derive-breakpoint-layout.ts` · `grid-slot.ts` · `migrate-card-grid.ts` · `card-background-upload.ts`
...
**عملکرد UI:** کارت‌ها و دیالوگ‌ها بدون `backdrop-filter` (پس‌زمینهٔ `rgba` ثابت). موتور شناور: کش `Path2D`، throttle ~۳۰fps در حالت آرام، حداکثر ۱۸ ذره، DPR سقف‌دار.
...
## ۱۰. تست package-cache — ماژول fixture

**مسیر:** [`tests/fixtures/modules/package-cache-test/`](../tests/fixtures/modules/package-cache-test/)

| مرحله | دستور / انتظار |
|--------|----------------|
| ساخت ZIP | `bash scripts/build-package-cache-fixture-zip.sh` (ویندوز: `build-package-cache-fixture-zip.ps1`) |
| آپلود ×۲ | `bash scripts/smoke/test-package-cache.sh` — upload2: `installed:false` · hash یکسان |
| wizard | Docker خیر · پورت **4100** · needsProcess بله → **Start** |
| UI | `/modules/<module-id>/` — diagnostics باید **PASS** |
| verify | `bash scripts/verify-package-cache.sh <module-id> [module-id-2]` |

hash ثابت fixture در README همان ماژول — unit: `tests/unit/package-cache/package-cache-fixture-hash.test.ts`
