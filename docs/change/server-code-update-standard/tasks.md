...
- [ ] ۱.۱ روی سرور کلید SSH بساز و در GitHub حساب `alishafiee1` اضافه کن
- [ ] ۱.۲ `origin` را به SSH عوض کن — تست: `git fetch origin` بدون سؤال رمز
- [ ] ۱.۳ همان remote را در `/opt/modulehub-cms` هم چک کن (اگر clone جداست)
...
- [ ] ۲.۱ از ویندوز تغییرات استانداردسازی اسکریپت را push کن
- [ ] ۲.۲ روی سرور: `bash scripts/deploy-full.sh --skip-wan --yes`
- [ ] ۲.۳ `bash scripts/run-checks.sh` — health OK
...
- [ ] ۳.۱ `git-wan-fetch.sh`: اگر default route از قبل آزاد است → خودکار `SKIP_WAN`
- [ ] ۳.۲ `deploy-full.sh`: پیش‌فرض `--skip-wan` وقتی `packageInstallInterface` = default route
- [ ] ۳.۳ پیام خطای واضح وقتی auth شکست خورد (راهنما به SSH/PAT)
...
- [ ] ۴.۱ `deploy-guide.md` — بخش «ورود مخزن» + «اینترنت آزاد = بدون WAN toggle»
- [ ] ۴.۲ `server-scripts.md` — به‌روز dual-WAN (فقط برای npm ماژول / docker)
- [ ] ۴.۳ `server condition.md` §۹ — جدول نقش ens4/enp63s0 بعد از flip
- [ ] ۴.۴ یک خط در `docs/tasks.md` اصلی — لینک به این change
...
- [ ] یک خط در `docs-personal/walkthrough.md`
