...
import { gridToPixels } from './grid.js';
...
const DEFAULT_GRID = { col: 0, row: 0, colSpan: 7, rowSpan: 3 };
...
/**
 * resolveNodeGrid --- cardGrid from layout node with fallback ---
 * @param {object} node
 * @param {number} index
 */
export function resolveNodeGrid(node, index = 0) {
  if (node.cardGrid) {
    return { ...node.cardGrid };
  }
  const colSpan = node.cardSpan === 4 ? 30 : node.cardSpan === 2 ? 15 : 7;
  const col = (index * colSpan) % GRID_CONFIG.maxColumns;
  const row = Math.floor((index * colSpan) / GRID_CONFIG.maxColumns) * GRID_CONFIG.minRowSpan;
  return { col, row, colSpan, rowSpan: GRID_CONFIG.minRowSpan };
}
...
export class ModuleHubCardStore {
  ...
  static fromLayoutNodes(children, modules) {
    return children.map((node, index) => {
      ...
      const grid = resolveNodeGrid(node, index);
      return {
        id: node.id,
        ...grid,
        nodeType: node.type,
  ...
  createCardElement(card, metrics, options = {}) {
    ...
    let statusHtml = '';
    if (moduleMeta) {
      const statusDisplay = this.getStatusDisplay(moduleMeta.status);
    ...
    root.dataset.cardBackground = escapeAttr(JSON.stringify(card.cardBackground || null));
