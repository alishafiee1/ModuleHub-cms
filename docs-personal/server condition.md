...
|------|-----|---------|--------|-----|
| **`enp63s0`** | `192.168.88.50/24` | `192.168.88.1` | **100** (اول) | اینترنت **آزادتر** — **پیش‌فرض سیستم** (از ۱۴۰۵/۰۳/۲۱) |
| **`ens4`** | `192.168.10.100/24` | `192.168.10.1` | **200** (دوم) | ADSL — ورود پورت‌فوروارد و نقاب |

```
مسیر پیش‌فرض سرور (خود Ubuntu):
  enp63s0 (192.168.88.50) metric 100  ←  apt / npm / git بدون metric موقت

ورود از ADSL + پاسخ:
  adsl-policy-routing.service  ←  table 200 + ip rule از 192.168.10.100
...
**اصل کلی (پایدار — بعد از flip ۱۴۰۵/۰۳/۲۱):**
- مسیر پیش‌فرض سرور روی `enp63s0` (اینترنت آزاد) است.
- `git pull` / `curl github` بدون metric موقت کار می‌کند.
- `docker pull` ممکن است هنوز به `auth.docker.io` نرسد (فیلتر سرویس) — جدا تست کن.
- Xray با `sendThrough` از `enp63s0` خارج می‌شود.
- ورود ADSL: `adsl-policy-routing.service` + `xray-policy-routing.service`.
